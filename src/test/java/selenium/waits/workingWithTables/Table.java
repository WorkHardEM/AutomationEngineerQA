...
public class Table {
  ...
  public List<Map<String, WebElement>> getRowsWithColumnsByHeadings() {
    ...
    for (List<WebElement> row : rowsWithColumns) {
      ...
      getRowsWithColumnsByHeadings().add(rowByHeadings);
    }
    return rowsWithColumnsByHeadings;
  }

  public String getValueFromCell(int rowNumber, String columnName) {
    List<Map<String, WebElement>> getRowsWithColumnsByHeadings = getRowsWithColumnsByHeadings();
    return getRowsWithColumnsByHeadings.get(rowNumber - 1).get(columnName).getText();
  }
